...
class GreetingMaker(object):
    ...
    def syncdata(self, json_input):
        try:
            with open("secondary_server_2.json", "r") as myfile:
                data = myfile.read()
            obj = json.loads(data)
        except:
            obj = {}

        f = open("secondary_server_2.json", "w")
        datetime = list(json_input.keys())[0]
        obj[datetime] = json_input[datetime]
        json.dump(obj, f, indent=4)
        f.close()
...
def sync(servername):
    try:
        ...
        unsynced = server.get_unsynced("secondary.server2_unsynced")
    except:
        return None
    if unsynced is None:
        return None
